...
";
    ...
    public function search($search) {
        $stmt = $this->pdo->prepare("SELECT id, screenName, profileImage, profileCover, bio FROM users WHERE screenName LIKE ? LIMIT 6");
...
//    }
    ...
    public function getLinkTheme($account_id) {
        $stmt = $this->pdo->prepare("SELECT * FROM links_theme WHERE account_id = :account_id");
        $stmt->bindParam(':account_id', $account_id);
        $stmt->execute();
